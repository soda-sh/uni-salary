...
class Ui_mainWindow(object):
    def setupUi(self, mainWindow):
        # define properties and objects (converter generates this)
        ...
        # code written below until retranslateUi method is mostly done by us (not generated by converter)
        # init the table
        ...
        # menu item for adding new professor
    ...
    # checks if paths of input files exist in inputFilesPath.txt to load them into object
    def checkInputFiles(self):
        try:
            file = open('inputFilesPath.txt', 'r')

        # if file doesn't exist, informs the user to add them
        except FileNotFoundError:
            q.tprint('لطفا فایل های ورودی را انتخاب کنید')
            return False

        # if file exists, loads the file paths into object
        else:
    ...
    def write_down(self):
        ...
        # checking if the UI table includes any records
        if updated_data == []:
            # inform the user UI table is empty
            q.tprint("هیچ جدولی ساخته نشده است")
            return
        else:
            # proceed to generate the output excel file
            filePath = QtWidgets.QFileDialog.getSaveFileName(None, 'Save File', '', 'Excel Files (*.xlsx)')
            # check if any name has been given to write
            if filePath == ("", ""):
                return
            # creating a workbook with the path user provided
